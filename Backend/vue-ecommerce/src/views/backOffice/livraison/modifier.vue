...
export default {
    ...
    methods :{
       ...
       async modifierLivraison(){
            
          await axios.put(`http://localhost:3500/gestion/livraison/modifier/${this.$route.params.id}`,this.livraison).then(()=>{
                this.$swal.fire(
                    'Success!',
                    'Livraison Modifié!',
                    'success'
    ...
    computed :{
       ...
       created(){
        axios.get(`http://localhost:3500/gestion/livraison/afficher/${this.$route.params.id}`).then((res)=>{
